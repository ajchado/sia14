        ...
        if request.session.has_key('user'):
            if request.method == 'POST':
                if 'btnCancel' in request.POST:
                    ...
                    update = Event.objects.filter(id = event_id).update(isCanceled=True)

                    return redirect('webapp:administrator_events')
                return redirect('webapp:landing')
            return redirect('webapp:landing')
        return redirect('webapp:landing')
        ...
        if request.session.has_key('user'):
            if request.method == 'POST':
                if 'btnReviews' in request.POST:
                    ...
                    return redirect('webapp:organizer')
                    ...
                    update = Event.objects.filter(id = event_id).update(event_name = event_name, event_description = event_description,
                                                                        event_type = event_type, event_fee = event_fee)
                    return redirect('webapp:organizer')
        return redirect('webapp:landing')
